...
- 修复“按署名”等 Cos 卡片网格加载封面时，逐张请求重复汇总整库筛选项、阻塞主进程的问题；图库就绪后直接响应图片请求，慢封面不阻挡其他封面返回。

## [3.4.1] - 2026-09-12

- 移除 Cos 套图卡片悬停时弹出的详情浮层。
- Cos 封面复用已加载的缩略图缓存，减少返回列表与滚动时的重复请求；加载失败自动重试一次，并显示明确的加载与失败状态。

## [3.4.0] - 2026-09-12

- Cos 收藏按原文件夹整包读取包装子目录，索引、封面和查看器共享媒体清单；兼容旧缓存并保留迁移前副本。
- 按稳定套图 ID 重连改名后的目录，旧标签重新解析当前位置；启动自动核对在线根目录，离线根保留摘要，在线根可独立刷新，读取失败不会覆盖上次可用索引。
- “按 Coser”调整为“按署名”，支持组织和发行来源；全部收藏新增套图/合集、类型、主题筛选，原始发行名和编号可搜索。
- 识别 JFIF、TIFF、HEIC 等图片；TIFF/HEIC 生成应用缓存预览，原文件保持不变，预览失败可定位原文件。
- 本版本仅处理图片，不接入视频；普通相簿继续沿用原有目录读取方式。

## [3.3.1] - 2026-09-04

- Cos 图库改用可恢复的语义 URL 导航，统一返回按钮与 Backspace 行为；搜索输入和图片查看器会优先消费按键，直接刷新套图深链也能恢复上级分类与套图标题。
- Cos 套图页改为单一嵌入式导航头，移除重复的物理目录面包屑、相邻相簿、收藏与设置入口，同时保留图片搜索、排序、密度、刷新、PictureView 和 Finder 操作。
- 角色、Coser、造型与套图卡片接入原有紧凑、标准、宽松三档密度，并在返回分类时恢复搜索、滚动位置和相册内更新的密度。
- Coser 默认列表将“其他”和“未知 Coser”固定置顶；修复造型搜索零结果仍显示“全部套图”、错误项目数、首次载入短暂显示空数据及虚拟卡片首屏空白的问题。
